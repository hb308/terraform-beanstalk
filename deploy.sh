...
"OptionName":"IamInstanceProfile","Value":"'"$ROLE"'"}]'
  	...
  	local TRUST="trust_policy_elasticbeanstalk.json"
		...
		if [ -z "$ROLE_DESCRIPTOR" ] ; then
  			...
  			aws iam create-role --role-name $ROLE \
  			                    --assume-role-policy-document file://scripts/iam/$TRUST
			...
			aws iam add-role-to-instance-profile
		  	...
		  	aws iam put-role-policy
		  	                    --role-name $ROLE
		  	                    --policy-name $ROLE
		  	                    --policy-document file://scripts/iam/$POLICY
		else
...
FILE="target/$2"
ENV=${3:-dev}
SOLUTION_STACK=${4:-64bit Amazon Linux 2016.09 v2.2.0 running Docker 1.11.2}
if [ $# -lt 2 ] ; then
    usage
fi
